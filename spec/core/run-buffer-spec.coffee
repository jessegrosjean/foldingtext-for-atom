RunBuffer = require '../../lib/core/run-buffer'
...
describe 'RunBuffer', ->
  ...
  describe 'Accessing Attributes', ->
    ...
    it 'finds longest effective range of attribute at location', ->
      range = {}
      runBuffer.getAttributeAtIndex('a', 0, null, range)
      range.should.eql(location: 0, length: 4)

      runBuffer.getAttributeAtIndex('a', 1, null, range)
      range.should.eql(location: 0, length: 4)

      runBuffer.getAttributeAtIndex('a', 2, null, range)
      range.should.eql(location: 0, length: 4)

      runBuffer.getAttributeAtIndex('a', 3, null, range)
      range.should.eql(location: 0, length: 4)

      runBuffer.getAttributeAtIndex('b', 4, null, range)
      range.should.eql(location: 2, length: 3)

      runBuffer.getAttributeAtIndex('b', 5, null, range)
      range.should.eql(location: 2, length: 3)

      runBuffer.getAttributeAtIndex('b', 6, null, range)
      range.should.eql(location: 5, length: 1)
